...
- `[client]` Vite startup and URL (default `http://localhost:8001/`)
...
- app/client: `8001`
- websocket server: `8011`

## Verify Server Connect/Disconnect Logs

Open the client URL shown in `[client]` logs (default `http://localhost:8001/`) to trigger a WebSocket connect.

You can also use this temporary smoke client in another terminal while stack is running:

```bash
node -e 'const ws=new WebSocket("ws://127.0.0.1:8011");ws.onmessage=(e)=>{console.log(e.data);setTimeout(()=>ws.close(),1000);};'
```
